...
import { LANDLORD_STRING, USER_ID_STRING, LANDLORD_ID_STRING, TENANT_ID_STRING } from '../../constants/firebaseConstants';
import { setLandlordId, setPersonalDetailsInformation, setTenantId } from '../../redux/actions/userActions';
import { FirebaseFirestoreTypes } from '@react-native-firebase/firestore';
import { docIdGenerator } from '../../backend/fireStoreActions';
import { useFormik } from 'formik';
import addressEditFormValidation from '../../components/form-validations/addressEditFormValidation';
...
const AddresDetails = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      batch.update(db.collection(LANDLORD_ID_STRING).doc(docId), { landlordId: docId })
      batch.commit();
  ...
  const requestLocation = async () => {
    try {
      ...
      geocodePosition(Location.latitude.toString(), Location.longitude.toString())
    }
    catch (error) {
      console.log(error)
    };
  ...
  return (
    <MainWithHeader onClickBack={() => navigation.goBack()} >
      <ScrollView horizontal={false} >
        ...
        />
        <ScrollView horizontal={true} scrollEnabled={false} contentContainerStyle={styles.horizontalScroll}>
