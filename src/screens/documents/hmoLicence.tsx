...
const HMO = () => {
    ...
    const { propertyComplianceDocuments: { hmoLicence }, propertyId } = useSelector((state: any) => state.property);
    ...
    const { handleChange, handleSubmit, handleBlur, values, errors, setFieldValue } = useFormik({
      ...
      onSubmit: async (values) => {
        ...
        await saveDataWithDocumentName('hmoLicence', collectionReference, values)
        dispatch(setPropertyCompliance({ hmoLicence: values }))
      }
    })
    return (

        <MainWithHeader title="HMO Licence"
        ...
        >
            <>
                ...
                />
                <Box mt={3}>
                    <DatePicker title='Please enter your certificate end date' setDate={setDate} />
                </Box>
                <Box mt={3}>
