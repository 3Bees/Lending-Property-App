...
import { optionData } from '../../assets/dataset/dataset';
...
const EPCReportScreen = () => {
  ...
  const { propertyComplianceDocuments: { epcReport }, propertyId } = useSelector((state: any) => state.property);
  ...
  const { handleChange, handleSubmit, handleBlur, values, errors, setFieldValue } = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      await saveDataWithDocumentName('epcReport', collectionReference, values)
  ...
  return (
    <MainWithHeader title="EPC Report"
    ...
    >
      <ScrollView>
        ...
        <RadioButton
          ...
          items={optionData}
          dir="row" onChange={handleChange} value={''} name={''} />
        <InputBox
          inputTitle="Please enter your 9-digit EPC code"
          dropdown={false} value={''} />
        <InputBox inputTitle="EPC Rating" dropdown={true} value={''} />

        <DatePicker title='Please enter your certificate issue date' onChange={
          handleChange('issueDate')
        } />
