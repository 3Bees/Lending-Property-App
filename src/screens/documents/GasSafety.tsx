...
import { BooleanData } from '../../assets/dataset/dataset';
...
import { saveDataWithDocumentName } from '../../backend/fireStoreActions';
import { PROPERTY_ID_STRING, PROPERTY_COMPLIANCE_DOCUMENTS } from '../../constants/firebaseConstants';
...
const GasSafety = () => {
  ...
  const { propertyComplianceDocuments: { gasSafety }, propertyId } = useSelector((state: any) => state.property);
  const [resourcePath, setResourcePath] = useState<Array<ImagePicker.ImagePickerResponse>>([]);
  ...
  const { handleChange, handleSubmit, handleBlur, values, errors, setFieldValue } = useFormik({
    ...
    onSubmit: async (values) => {
      const collectionReference = db.collection(PROPERTY_ID_STRING).doc(propertyId.propertyId).collection(PROPERTY_COMPLIANCE_DOCUMENTS)
      await saveDataWithDocumentName('gasSafety', collectionReference, values)
      dispatch(setPropertyCompliance({ gasSafety: values }))
    }
  })
  return (
    <MainWithHeader title="Gas safety certificate"
    ...
    >
      <>
        <RadioButton
          ...
          items={BooleanData}
          dir="row" onChange={handleChange} 
          value={values.exemption} 
          name={'exemption'}
        />
        <InputBox
          inputTitle="Please enter your Gas Safe ID"
          dropdown={false} value={values.number}
          onChange={handleChange('number')} />
        <InputBox inputTitle="Please enter your registered engineer's name/number" onChange={handleChange('reference')}
          dropdown={true} value={values.reference} />
        <Box>
        <DatePicker title='Please enter your certificate issue date' setDate={setCerDate} />
        </Box>
        <Box mt={3}>
        <DatePicker title='What is the next inspection due date' setDate={setDate} />
        </Box>
        ...
        <View style={{ marginBottom: 10 }}>
          <Container alignSelf="center">
            <Box
              ...
              flexWrap="wrap">
              {resourcePath && resourcePath.map((item: ImagePicker.ImagePickerResponse) => {
