...
const userReducer = (state = intialState, action: any) => {
  switch (action.type) {
    ...
    case SET_PERSONAL_DETAILS_INFORMATION:
      return { ...state, userId:{...state.userId ,title:action.payload.title, firstName:action.payload.firstName,lastName:action.payload.lastName, dob:action.payload.dob,
      }  };
    case SET_PERSONAL_DETAILS_ADDRESS:
      return {  ...state,  userId:{...state.userId ,userFullAddress:action.payload}};
      case SET_TENANTID:
        return{...state, userId:{...state.userId, tenantId:action.payload.tenantId}}
        case SET_LANDLORDID:
          return{...state, userId:{...state.userId, landlordId:action.payload.landlordId}}
