...
const rentalOfferValidation = (values: any,data: any) => {
    ...
    if (values.totalTenants >data.maxTenants) {
        errorList.totalTenants = "Sorry the property is not suitable for the amount of tenants selected(" + data.maxTenants+ ")"
    
    }
    
    if (diffDays > data.advertisedMoveInDate) {
        errorList.moveIn = 'Move In Date needs to be within 60 days of the property Move In Date'
    }
    ...
    if (values.rent < data.advertisedMonthlyRent / 2) {
        errorList.rent = "Sorry this offer is much lower than advertised  " + data.advertisedMonthlyRent  + ")"
