...
import {GUARANTOR_ID_STRING, LANDLORD_ID_STRING, LANDLORD_STRING, TENANT_ID_STRING, USER_ID_STRING,kycFields} from '../constants/firebaseConstants';
import { MISSING_CONTACT_DETAILS_STRING, MISSING_PAYMENT_METHOD_STRING, MISSING_PERSONAL_DETAILS_STRING, } from "../constants/notifcationsConstants";
...
export const checkLoginData = async (userId,link)=>{
  const userDocSnap = await getUserSnap(userId)
  if(userDocSnap){
    const [matches] = await checkDocumentFields(userDocSnap,{userId})
    if(matches.userId){
     return link('App')
    }
  }
  return link('personaldetails')
}

// export const getUserSnap = async (userId) => {
